...
if __name__ == '__main__':
    ...
    if args.walltime:
        ...
        for i in range(len(namespace)):
            if args.all:
                name_n_val.append((namespace[i], float(values[i][1])))
            else:
        ...
        for i in range(len(g_namespace)):
            if args.all:
                name_n_val.append((g_namespace[i], float(g_values[i][1])))
            else:
        ...
        for i in range(len(c_namespace)):
            if args.all:
                name_n_val.append((c_namespace[i], float(c_values[i][1])))
            else:
        ...
        for i in range(len(m_namespace)):
            ...
            if args.all:
                name_n_val.append((m_namespace[i], float(m_values[i][1])))
            else:
